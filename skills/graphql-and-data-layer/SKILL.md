...
description: Implements Apollo Client 3.8 patterns for StackRox UI's GraphQL surface. Use when modifying existing GraphQL queries or mutations, building a UI for a new GraphQL endpoint the backend team is shipping, managing the Apollo cache (reads, writes, evictions), implementing optimistic updates, or debugging stale data or unexpected cache behavior. NOT for new-feature data fetching that does not already involve GraphQL (see `rest-and-service-layer`), state management decisions (see `react-state-patterns`), or E2E interception of GraphQL traffic (see `cypress-e2e-testing`).
---

# GraphQL and Data Layer

For new features that fetch or mutate REST data, use the `rest-and-service-layer` skill instead. This skill covers the GraphQL subset only.

## Overview

StackRox UI is REST-first via `rest-and-service-layer`. Apollo Client 3.8 is used for a bounded GraphQL surface: existing GraphQL queries and mutations already in the codebase, plus the occasional new GraphQL endpoint the backend team ships for a specific feature. This skill covers typed query and mutation patterns, cache management, optimistic updates, and error and loading states for that GraphQL subset.

## When to Use

- Modifying an existing GraphQL query or mutation
- Building UI for a new GraphQL endpoint the backend team is adding for this feature
- Managing Apollo cache for GraphQL data (reads, writes, evictions, merge policies)
- Implementing optimistic updates for a predictable GraphQL mutation
- Debugging stale data or unexpected Apollo cache behavior
- NOT for new-feature data fetching that does not already involve GraphQL (see `rest-and-service-layer`)
...
Is the feature fetching or mutating server data?
  │
  ├── Are you modifying an existing GraphQL query or mutation?
  │     YES --> Use Apollo Client (this skill)
  │     NO  --> continue
  │
  ├── Is the backend team shipping a new GraphQL endpoint
  │   specifically for this feature?
  │     YES --> Use Apollo Client (this skill)
  │     NO  --> continue
  │
  └── Use REST via `rest-and-service-layer`.
```

REST is the default. Reach for Apollo only on the two triggers above. Adding new GraphQL queries against an unchanged backend is not a supported path and will be flagged in review.
...
## Service Layer for REST

For REST patterns (service layer structure, `useRestQuery` / `useRestMutation` / `usePaginatedQuery`, cancellation, search, pagination, error formatting), see `rest-and-service-layer`. Do not duplicate REST examples here.
...
| "I'll add a new GraphQL query for this new feature" | GraphQL is reserved for existing GraphQL code and new GraphQL endpoints the backend team is shipping. New-feature data fetching goes through `rest-and-service-layer`. |
...
- New GraphQL queries or mutations added to the UI when the backend has not shipped a matching new GraphQL endpoint
...
- [ ] New-feature data fetching goes through `rest-and-service-layer`, not new GraphQL queries
